...
Before proceeding to building a model, I would like to examine the data by examining specific informations.
...
2. Who are the best non-post shooters for the first quarter, last quarter and the overtime? This gives us an idea who are the best to start and end the game.
...
ggplot(data = home_away, aes(x = LOCATION, y = PERCENTAGE, fill = LOCATION)) + geom_bar(stat = 'identity') + theme_bw()
...
ggplot(data = wins, aes(LOCATION, fill = factor(W)) ) + geom_bar() + theme_bw()
...
## 2. Best Starter and Finisher - non-post players (shots 5ft+ away from the basket)
...
```
Not surprising, Marc Gasol is the top player.

## 3. Top End Game Players
...
```{r, warning = F, message = F}
end_game <- nba_shots
end_game$SHOT_CLOCK[is.na(end_game$SHOT_CLOCK)] <- 0
...
ggpairs(shots_data[sample, -c(9,11)], aes(color = SHOT_RESULT, alpha = 0.75)) + theme_minimal()
```
