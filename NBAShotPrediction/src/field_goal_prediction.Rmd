...
```{r, warning = F, message = F, fig_width = 5, fig_height = 3}

home_away <- nba_shots %>% group_by(LOCATION) %>% summarize(PERCENTAGE = (sum(FGM)/length(FGM))*100)
ggplot(data = home_away, aes(x = LOCATION, y = PERCENTAGE, fill = LOCATION)) + geom_bar(stat = 'identity') + theme_minimal()
...
ggplot(data = wins, aes(LOCATION, fill = factor(W)) ) + geom_bar() + theme_minimal()
