...
final2 <- read.csv("./input/formatted_v2.csv")
...
for (i in sessions){
	...
	write.xlsx(data, paste("./output/data.xlsx"), sheetName = i, append = TRUE, row.names = FALSE)
...
library(readxl)
length(excel_sheets("./output/data.xlsx")) ## to check if all are saved accordingly - match the number to the actual number of sessions/shows
